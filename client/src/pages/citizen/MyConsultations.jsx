...
// Chat stays open for accepted consultations, and for 48h after completion —
// matches the server-side window enforced in consultationChat.controller.js.
const CHAT_RETENTION_HOURS = 48;
function isChatOpen(consultation) {
  if (consultation.status === 'accepted') return true;
  if (consultation.status === 'completed' && consultation.completedAt) {
    const hours = (Date.now() - new Date(consultation.completedAt).getTime()) / (1000 * 60 * 60);
    return hours <= CHAT_RETENTION_HOURS;
  }
  return false;
}
...
function MyConsultations() {
  ...
  const loading = useSelector(selectConsultationsLoading);
