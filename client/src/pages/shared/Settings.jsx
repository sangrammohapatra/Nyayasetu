  ...
  const items = [
    {
      ...
      icon: "⚖️",
      label: t("settings.notif.hearings", "Hearing Reminders"),
      desc: "Get alerted 2 days before each scheduled hearing",
    },
    {
      key: "documentReady",
      icon: "📄",
      label: t("settings.notif.docReady", "Document Ready"),
      desc: "When your generated document is ready",
    },
    {
      key: "weeklyDigest",
      icon: "📊",
      label: t("settings.notif.digest", "Weekly Digest"),
      desc: "Summary of cases and upcoming hearings",
    },
  ];
    ...
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
      ...
      >
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1.75 }}>
          {meters.map((m) => {
            const pct =
              m.limit === 999999 ? 5 : Math.min(100, (m.used / m.limit) * 100);
...
function Settings() {
  ...
  const handleSave = async (data) => {
    ...
    if (result.meta.requestStatus === "fulfilled") {
      ...
      showSnack(
        t("settings.saveFailed", "Failed to save. Please try again."),
        "error",
      );
    }
  };
