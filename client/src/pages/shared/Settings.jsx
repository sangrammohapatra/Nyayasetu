...
const SETTINGS_ICON_SRC = "https://cdn.lordicon.com/asyunleq.json";
...
function ProfileHero({ user, persona }) {
  ...
  const personaLabel = persona ? persona.charAt(0).toUpperCase() + persona.slice(1) : "User";
      ...
      <Box sx={{
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.75 }}>
            <Chip label={personaLabel} size="small"
              sx={{ background: "var(--color-primary)", color: "#fff", fontWeight: 600, fontSize: "0.72rem" }} />
            <Chip label={`${planLabel} Plan`} size="small" variant="outlined"
              sx={{ borderColor: "var(--color-border)", color: "var(--color-text-secondary)", fontSize: "0.72rem" }} />
            <Chip label={`Member since ${formatMemberSince(user?.createdAt)}`} size="small" variant="outlined"
              sx={{ borderColor: "var(--color-border)", color: "var(--color-text-secondary)", fontSize: "0.72rem" }} />
          </Box>
        </Box>
      </Box>
...
function AccountSection({ user, showSnack }) {
  ...
  useEffect(() => {
    ...
    setLocation({ state: user.state || "", district: user.district || "", pincode: user.pincode || "" });
  }, [user]);

  const savePersonal = async () => {
    setPersonalBusy(true);
    const result = await dispatch(updateMe({
    ...
    updateMe.fulfilled.match(result)
      ...
      : showSnack(result.payload || t("settings.save_failed", "Failed to save. Please try again."), "error");
  };
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
      <Typography variant="h6"
        sx={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, color: "var(--color-text)" }}>
      ...
      <Box sx={boxSx}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6}>
            <TextField fullWidth size="small" label={t("settings.name", "Full Name")}
              value={personal.name} onChange={(e) => setPersonal((p) => ({ ...p, name: e.target.value }))}
              sx={fieldSx} />
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField fullWidth size="small" label={t("settings.email", "Email (optional)")} type="email"
        ...
        <Button variant="contained" onClick={savePersonal} disabled={personalBusy || !personal.name.trim()}
          sx={{ mt: 2.5, borderRadius: `${RADIUS.md}px`, fontWeight: 700, background: "var(--color-primary)" }}>
          {personalBusy
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6}>
            <FormControl fullWidth size="small">
              ...
              <Select value={location.state} label={t("settings.state", "State")}
          ...
          <Grid item xs={12} sm={6}>
            <TextField fullWidth size="small" label={t("settings.district", "District")}
              value={location.district} onChange={(e) => setLocation((p) => ({ ...p, district: e.target.value }))}
              sx={fieldSx} />
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField fullWidth size="small" label="Pincode"
              value={location.pincode}
              onChange={(e) => setLocation((p) => ({ ...p, pincode: e.target.value.replace(/\D/g, "").slice(0, 6) }))}
              inputProps={{ maxLength: 6, inputMode: "numeric" }}
              sx={fieldSx} />
          </Grid>
        </Grid>
        <Button variant="contained" onClick={saveLocation} disabled={locationBusy}
          sx={{ mt: 2.5, borderRadius: `${RADIUS.md}px`, fontWeight: 700, background: "var(--color-primary)" }}>
          {locationBusy
            ? <CircularProgress size={20} sx={{ color: "#fff" }} />
            : "Save Location"}
...
function AppearanceSection() {
  ...
  return (
    <Box>
      <Typography variant="h6"
        sx={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, color: "var(--color-text)", mb: 2 }}>
        {t("settings.appearance", "Appearance")}
      </Typography>
      <Typography variant="body2" sx={{ color: "var(--color-text-secondary)", mb: 2.5 }}>
        {t("settings.theme.desc", "Choose a theme that suits you. Changes take effect immediately.")}
      </Typography>
        ...
        {THEMES.map((th) => {
          ...
          return (
            <motion.div key={th.id} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
              <Box onClick={() => dispatch(setTheme(th.id))} sx={{
                ...
                <Box sx={{ height: 70, background: th.bg, display: "flex", flexDirection: "column", gap: 0.75, p: 1.5 }}>
            ...
            <Chip key={lang.code}
              label={
                <Box sx={{ textAlign: "center" }}>
                  <span style={{ display: "block", fontSize: "0.85rem", fontWeight: 700 }}>{lang.native}</span>
                  <span style={{ fontSize: "0.65rem", opacity: 0.75 }}>{lang.label}</span>
                </Box>
              }
...
function NotificationsSection({ user, showSnack }) {
  ...
  const notifItems = [
    {
      ...
      desc: t("settings.notifications.hearings_desc", "Get alerted 2 days before each scheduled hearing"),
    },
    {
      key: "documentReady",
      icon: "📄",
      label: t("settings.notifications.doc_ready", "Document Ready"),
      desc: t("settings.notifications.doc_ready_desc", "When your generated document is ready"),
    },
    {
      key: "weeklyDigest",
      icon: "📊",
      label: t("settings.notifications.digest", "Weekly Digest"),
      desc: t("settings.notifications.digest_desc", "Summary of cases and upcoming hearings"),
    },
  ];
  ...
  const switchSx = { "& .MuiSwitch-thumb": { background: "var(--color-primary)" } };
  const rowSx = {
    mb: 1.5, display: "flex", width: "100%",
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
      ...
      <Box sx={{ p: 2, borderRadius: `${RADIUS.lg}px`, border: "1px solid var(--color-border)", background: "var(--color-surface)" }}>
        ...
        <FormControlLabel sx={{ ...rowSx, mb: prefs.whatsapp ? 1 : 1.5 }}
          labelPlacement="start"
          control={<Switch checked={prefs.whatsapp} onChange={() => toggle("whatsapp")} sx={switchSx} />}
          label={
            <Box>
        ...
        {prefs.whatsapp && (
          <TextField fullWidth size="small" label="WhatsApp Number"
            ...
            onChange={(e) => setPrefs((p) => ({ ...p, whatsappNumber: e.target.value.replace(/\D/g, "").slice(0, 10) }))}
            inputProps={{ maxLength: 10, inputMode: "numeric" }}
            InputProps={{ startAdornment: <InputAdornment position="start"><Typography variant="caption" sx={{ color: "var(--color-text-secondary)" }}>+91</Typography></InputAdornment> }}
            sx={{ mb: 1.5, "& .MuiOutlinedInput-root": { borderRadius: `${RADIUS.md}px` } }}
          />
        )}

        <FormControlLabel sx={rowSx} labelPlacement="start"
          control={<Switch checked={prefs.email} onChange={() => toggle("email")} sx={switchSx} />}
          label={
            <Box>
      ...
      <Box sx={{ p: 2, borderRadius: `${RADIUS.lg}px`, border: "1px solid var(--color-border)", background: "var(--color-surface)" }}>
        ...
        {notifItems.map((item) => (
          <FormControlLabel key={item.key} labelPlacement="start" sx={rowSx}
            control={<Switch checked={prefs[item.key]} onChange={() => toggle(item.key)} sx={switchSx} />}
            label={
              <Box sx={{ mb: 0.25 }}>
                <Typography variant="body2">{item.icon} {item.label}</Typography>
                <Typography variant="caption" sx={{ color: "var(--color-text-secondary)" }}>{item.desc}</Typography>
              </Box>
            }
          />
        ))}
    ...
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
      ...
      <Box sx={{ p: 2.5, borderRadius: `${RADIUS.xl}px`, border: "2px solid var(--color-primary)", background: "var(--color-primary-alpha)" }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
          <Box>
            ...
            <Typography variant="h5" sx={{ fontWeight: 800, color: "var(--color-primary)", textTransform: "capitalize" }}>
              {plan}
            </Typography>
          </Box>
          {plan === "free" && (
            ...
            return (
              <Box key={m.label}>
                ...
                <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8, ease: "easeOut" }} style={{ transformOrigin: "left" }}>
                  <LinearProgress variant="determinate" value={unlimited ? 10 : pct} sx={{
                    height: 6, borderRadius: 3, background: "var(--color-border)",
                    "& .MuiLinearProgress-bar": {
                      background: pct >= 90 && !unlimited ? "var(--color-error)" : pct >= 70 && !unlimited ? "var(--color-warning)" : "var(--color-primary)",
                      borderRadius: 3,
                    },
                  }} />
                </motion.div>
        ...
        {freeUsage?.resetDate && (
          <Typography variant="caption" sx={{ display: "block", color: "var(--color-text-secondary)", mt: 1.5 }}>
      ...
      {plan !== "free" && (
        <Button variant="outlined" size="small"
          onClick={async () => {
            if (window.confirm("Cancel subscription? You keep access until end of billing period.")) {
              await dispatch(cancelSubscription());
            }
          }}
          sx={{ borderRadius: `${RADIUS.md}px`, borderColor: "var(--color-error)", color: "var(--color-error)", fontWeight: 600, width: "fit-content" }}>
...
function SecuritySection({ showSnack }) {
  ...
  const handleChangePassword = async () => {
    ...
    setPwBusy(true);
    const payload = { password: pw.next, ...(pw.current && { currentPassword: pw.current }) };
  ...
  const eyeBtn = (key) => (
    <InputAdornment position="end">
      <IconButton size="small" onClick={() => setShowPw((p) => ({ ...p, [key]: !p[key] }))}>
        <Typography sx={{ fontSize: 16 }}>{showPw[key] ? "🙈" : "👁️"}</Typography>
      </IconButton>
    </InputAdornment>
  );
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
      ...
      <Box sx={{ p: 2.5, borderRadius: `${RADIUS.lg}px`, border: "1px solid var(--color-border)", background: "var(--color-surface)" }}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <TextField fullWidth size="small" label="Current Password (leave blank if not set)"
              type={showPw.current ? "text" : "password"} value={pw.current}
              onChange={(e) => setPw((p) => ({ ...p, current: e.target.value }))}
              InputProps={{ endAdornment: eyeBtn("current") }} sx={fieldSx} />
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField fullWidth size="small" label="New Password"
              type={showPw.next ? "text" : "password"} value={pw.next}
              helperText="Min. 8 characters"
              onChange={(e) => setPw((p) => ({ ...p, next: e.target.value }))}
              InputProps={{ endAdornment: eyeBtn("next") }} sx={fieldSx} />
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField fullWidth size="small" label="Confirm New Password"
              type={showPw.confirm ? "text" : "password"} value={pw.confirm}
              onChange={(e) => setPw((p) => ({ ...p, confirm: e.target.value }))}
              InputProps={{ endAdornment: eyeBtn("confirm") }} sx={fieldSx} />
          </Grid>
        </Grid>
        <Button variant="contained" onClick={handleChangePassword} disabled={pwBusy || !pw.next}
          sx={{ mt: 2.5, borderRadius: `${RADIUS.md}px`, fontWeight: 700, background: "var(--color-primary)" }}>
          {pwBusy ? <CircularProgress size={20} sx={{ color: "#fff" }} /> : "Change Password"}
        </Button>
      </Box>
      ...
      <Box sx={{ p: 2.5, borderRadius: `${RADIUS.lg}px`, border: "1px solid var(--color-border)", background: "var(--color-surface)" }}>
        ...
        <Button variant="outlined" size="small" onClick={handleLogoutAll}
      ...
      <Box sx={{ p: 2.5, borderRadius: `${RADIUS.lg}px`, border: "1px solid var(--color-border)", background: "var(--color-surface)" }}>
        ...
        <Typography variant="caption" sx={{ color: "var(--color-text-secondary)", display: "block", mb: 2, lineHeight: 1.7 }}>
      ...
      <Dialog open={deactivateOpen} onClose={() => !deactivateBusy && setDeactivateOpen(false)}
        ...
        <DialogContent>
          <Typography variant="body2" sx={{ color: "var(--color-text-secondary)", lineHeight: 1.7 }}>
            This will immediately sign you out. Your documents and case data will be retained for 30 days.
            Contact <strong>support@nyayasetu.in</strong> to reverse this.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
          <Button onClick={() => setDeactivateOpen(false)} disabled={deactivateBusy} sx={{ borderRadius: 2 }}>
            Cancel
          </Button>
          <Button variant="contained" color="error" onClick={handleDeactivate} disabled={deactivateBusy}
            sx={{ borderRadius: 2, fontWeight: 700 }}>
            {deactivateBusy ? <CircularProgress size={18} sx={{ color: "#fff" }} /> : "Yes, Deactivate"}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
...
function Settings() {
  ...
  return (
    <AnimatedPage>
      <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, maxWidth: 960, mx: "auto", pb: { xs: 10, md: 6 } }}>
            ...
            <Box sx={{ width: 200, flexShrink: 0 }}>
              <Box sx={{ borderRadius: `${RADIUS.xl}px`, border: "1px solid var(--color-border)", background: "var(--color-surface)", overflow: "hidden" }}>
                {SECTIONS.map((sec) => {
                  ...
                  return (
                    <Box key={sec.id} onClick={() => setActiveTab(sec.id)} sx={{
            ...
            <Box sx={{ flex: 1 }}>
              <AnimatePresence mode="wait">
                <motion.div key={activeTab}
                  ...
                  <Box sx={{ p: 3.5, borderRadius: `${RADIUS.xl}px`, border: "1px solid var(--color-border)", background: "var(--color-surface)", boxShadow: SHADOWS.sm }}>
